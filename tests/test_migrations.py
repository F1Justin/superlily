...
def test_sqlite_alembic_upgrade_reaches_control_plane_head_and_round_trips(
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        platform_api_call_columns = {
            row[1] for row in connection.execute("PRAGMA table_info(platform_api_calls)").fetchall()
        }

    assert version == ("0033_history_delivery_receipts",)
    ...
    assert agent_triggers == {
        ...
        "agent_model_profiles_no_delete",
        "agent_runs_authority_no_update",
        "agent_runs_no_delete",
        "agent_runs_state_guard",
        "agent_run_events_no_update",
        "agent_run_events_no_delete",
        "agent_run_attempts_no_update",
...
def test_postgres_alembic_control_plane_round_trip_and_drift() -> None:
    ...
    try:
        ...
        alembic("upgrade", "head")
        assert asyncio.run(snapshot())[0] == "0033_history_delivery_receipts"
    finally:
