...
状态：已部署，展开与下载未启用，真实媒体归档闭环尚未验证（2026-09-12 复核）。服务 MANIFESTO 第 2、3 条，
...
- Nekro 生产插件配置 `MEDIA_ARCHIVE_ENABLED=false`、`MEDIA_DOWNLOADS_ENABLED=false`；
  `HISTORY_RECOVERY_ENABLED=true` 属于 R5.4，不代表媒体归档也已启用。
- Lily `.env.prod` 未配置启用两项媒体开关，已部署桥接器两项默认值均为 false；
  历史补采已启用与此独立。
- 生产 `qq_media_archive_items`、`qq_media_blobs` 均为 0 条。

因此，已有自动化、隔离数据库验证及生产部署检查，但没有本功能的真实媒体归档入库证据。
不能用桥接器已在线数日、普通消息含媒体段或 C0-E 群文件通知入库替代本阶段的展开、下载、
上传与脱离原平台 URL 读回内容验证。后续受控启用仍需明确授权，本次未改动开关或发起下载。
