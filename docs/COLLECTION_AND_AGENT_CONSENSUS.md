...
状态：2026-07-18 用户确认的采集与归档设计共识。C0-D 已完成生产签署；本文继续
保留其历史设计和 C0-A 档案扩展边界。群聊 Agent、工具选择、渐进式披露、模型路由、
自然语言命令和输出策略已经移至
`AGENT_PRODUCT_AND_IMPLEMENTATION_CONSENSUS.md`。具体生产权限仍以
`ROADMAP.md`、阶段验收和 accepted ADR 为准。
...
此前混在本文中的快速回复、模型自主选择、渐进式披露、Unix 原语、自然语言命令、
模型路由、reaction 反馈语义、渲染输出和未知入口设计，现统一由
`AGENT_PRODUCT_AND_IMPLEMENTATION_CONSENSUS.md` 维护。本文不再承担 Phase 5
以后 Agent 能力的实施指导，避免已完成的采集工程与尚未开放的模型 authority 混淆。
