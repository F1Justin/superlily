...
### R5.1（C0-E）：QQ 平台事实完整性（实现完成，待生产验收）
...
### R5.4（C0-H）：断线补洞（两端已启用，Nekro 小窗口验收通过，待全范围验收）
...
私聊断线样本和全范围完整性尚未验收，不将小窗口 canary 或
补采任务结束视为全部阶段签署。
...
### R5.5（C0-I）：媒体与合并转发补全（已部署，未启用，待生产验收）
...
第 2、3 项的生产签署见各自阶段文档；第 1 项仍需补齐生产验收记录。R5.4 已完成 Nekro
小窗口断线实测，仍需 Lily/私聊等全范围验收；R5.5 已部署，下一步为受控启用与真实样本验收。
