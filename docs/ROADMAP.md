...
以下基础结论记录于 2026-09-04；C0-E/H/I 与节日皮肤的生产状态于 2026-09-12 单独复核，见本节补充及
各阶段记录，不表示本次重新验收了所有阶段。
...
- 三份 2022–2023 年 SQLite chatrecorder 冻结快照已纳入生产 archive：741,964 条消息
  成功进入 `archive.message_timeline_v2`，221 条因无法证明私聊对端而保留在 rejected
  ledger；生产 Core 已迁移到 `0028_sqlite_chatrecorder_archive`；
...
- 当前处于 R3 的现有 Runtime 演进方向，但没有正在推进的产品切片。R3.1 Web Search
  技术链路和管理界面开关已实现，因真实请求费用未通过产品门而暂停，所有生产模型组保持
  关闭；
- 当前没有自动排队的“下一阶段”。恢复 R3.1 或建立新的 R3/R5 切片，都必须由新的具体
  产品需求和明确决定触发；
- 详细可复核身份、数据库和成本数据见 [`R0_BASELINE.md`](R0_BASELINE.md)。

2026-09-12 C0-E/H/I 生产复核：

- R5.1（C0-E）已上线并有真实运行验证：新增范围内已有 422 条平台事件观测，待补齐
  全类型覆盖和阶段验收记录，不是尚未部署或没有真实样本。
- R5.4（C0-H）两端自动补采已启用，Nekro 受控断线和 Lily 午夜断档均已有真实补采证据；
  部分生产场景已验证，私聊断线等场景仍待补齐，平台不可恢复的缺口继续保留。
- R5.5（C0-I）代码与迁移已部署，但展开/下载未启用；两张媒体归档表均为 0 条。
  自动化与部署检查已完成，真实媒体归档闭环尚未验证。
- R1.2 节日渲染皮肤已于 9 月 9 日部署并启用，技术验证完成；9 月 12 日生产
  仍按日历返回 default，首个真实节日窗口为 9 月 25 日。专项记录见下文，不是未实施阶段。

2026-09-14 R1 Renderer 补充：多文字字体回退、阿拉伯文字段连写/方向及单色 emoji 已部署，
真实成图与生产下载/缓存验证通过；见 [多文字渲染记录](MULTILINGUAL_RENDERER_20260914.md)。

2026-09-16 [R1.2：节日渲染皮肤](#r1-2)：16 款已部署启用，技术验证通过；
待 9 月 25 日首个真实节日窗口验证。独立范围、排期与验收状态见该条目。
...
- 已于 2026-09-16 完成扩展和跨年窗口调整并部署启用，保留既有多文字字体修复。
  ...
  到期更新、生产下载与服务健康均已验证。
- 尚待验证：首个真实节日窗口为 2026-09-25 中秋。模拟日期与内部成图验证不等于
  节日当天群聊效果已经验收；验证过程没有发送平台测试消息。
...
### R5.1（C0-E）：QQ 平台事实完整性（已上线，有真实运行验证，待补齐验收覆盖）
...
### R5.4（C0-H）：断线补洞（两端已启用，部分生产场景已验证，待补齐验收覆盖）
...
第 2、3 项的生产签署见各自阶段文档；第 1 项已有真实运行样本，仍需补齐验收覆盖与记录。
R5.4 已完成 Nekro 小窗口断线实测及 Lily 一轮真实断档补采，剩余私聊断线等场景尚待验证，
已知不可恢复缺口不抹除；R5.5 已部署但未启用，下一步须经授权受控启用并验收真实样本。
