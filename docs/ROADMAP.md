...
### R5.5（C0-I）：媒体与合并转发补全（已纳入路线图，待实施）
...
第 2、3 项的生产签署见各自阶段文档；第 1 项仍需补齐生产验收记录。后续先推进 R5.4，
再推进 R5.5；本次仅补齐路线图，实施时再细化合同、迁移与生产验收方案。ChatExporter
保持不变，数据库和归档结构继续满足既有导出读取约定。
