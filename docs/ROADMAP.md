...
- R1 Renderer 的节日皮肤增强已于 9 月 9 日部署并启用，技术验证完成；9 月 12 日生产
...
### R1 已交付增强：节日渲染皮肤（已部署启用，待真实节日窗口验证）

元旦／跨年皮肤窗口为北京时间 2026-12-31 12:00 至 2027-01-01 12:00，见 [跨年调整记录](NEW_YEAR_WINDOW_20260916.md)。

节气设计范围：仅春季六节气做系列，现已全部上线；保留已有冬至皮肤。其他季节的节气不列为制作计划，后续从传统节日、中外节日和纪念日中挑选。

2026-09-16 已在后续生产镜像上扩展至 16 款：新增万圣夜、圣诞与春季六节气；
保留多文字修复，八款批准成图与生产输出逐一一致，97 项回归通过。
当前发布身份和回滚见 [FESTIVAL_EXPANSION_20260916.md](FESTIVAL_EXPANSION_20260916.md)，
[节日清单](FESTIVAL_CALENDAR_20260916_20270914.md)覆盖至 2027 中秋前。以下为初次发布沿革。

作为既有 Renderer 的确定性展示增强登记，不新建自动排队阶段，也不改变 R1 原有认知反馈
验收范围。用户已确认的八款纯图形装饰覆盖 2026 中秋至 2027 元宵；正文和数学仍由
XeLaTeX/Poppler 渲染，不增加模型提示词、工具参数或装饰文字。

2026-09-09 已完成自动化、隔离真实成图及生产 Core → gateway → worker → artifact
链路验证并启用日期表；9 月 12 日复核开关仍为 true，worker 身份与原发布一致。
当前 default 是非节日的正常结果，9 月 25 日中秋才进入第一个配置窗口；模拟时钟测试
不等于真实节日当天群聊效果已经验收。原发布未发 QQ 测试消息。

Core 已由 9 月 12 日消融镜像承接，保留节日能力；节日发布锁的原 images 是历史发布
快照，不应拿旧 Core 镜像覆盖后续改动。实现、日期、证据与分层回滚见
[FESTIVAL_RENDERER_20260909.md](FESTIVAL_RENDERER_20260909.md)及
[ABLATION_20260912.md](ABLATION_20260912.md)。
