...
状态：已实现、两端自动补采已启用（2026-09-19 口径整理）。Nekro 小窗口断线 canary
通过，Lily 真实午夜断档已完成一轮补采并保留缺口；未测场景是覆盖说明，不另挂阶段签署待办。
下文保留 2026-09-12 复核证据，不承诺全量历史完整恢复。
服务 MANIFESTO 第 2、3 条。

## 当前验收口径（2026-09-12）

- 已有生产证据：9 月 7 日 Nekro 约 61 秒受控断线，共同群核心窗口 4/4 补回，含重连
  等待共 8 条；9 月 8 日 Lily 实际断档补采新增 879 条历史 observation，其中 863 条
  属于午夜断档，另 48 个重复投递包通过独立回执确认。具体范围和对账见下文原始记录。
- 当日约 11:52 CST 只读核对生产 `qq_history_recovery_progress`，按
  `(instance_id, job_id)` 取最大 `revision`：Lily 96 个任务（54 scanned、42 partial），
  Nekro 305 个任务（210 scanned、95 partial）。进度记录延续到 9 月 9 日；两端补采
  配置均已启用，核查时两端心跳均为 online。
- 这些任务含 system discovery、bootstrap 和会话补采，不等于同数量的断线测试或独立
  会话。`partial` 包括有界发现、首次启动窗口、空历史、接口失败和回看上限；不能全算作
  功能故障。`scanned` 也不代表已经证明平台没有遗漏。
- 未覆盖的是可对照的私聊断线样本、NapCat 自身离线等生产场景；
  Lily 已有真实补采证据，不能再笼统列为“Lily 尚未验证”。平台保留期和接口能力造成的
  不可恢复缺口继续如实记录，不将“全范围验收”解释成保证恢复所有消息。

本次复核未制造新断线或补采任务；历史消息数量及无 claim/回复结论引用下文已记录的
对应测试窗口，不扩展为对所有后续任务的重新验收。
