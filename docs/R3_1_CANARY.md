...
状态：**技术链路可用；真实请求费用未通过产品门，项目暂停，生产保持关闭。**
...
证明完整 social epistemics。** 51 条真实回放通过只是必要条件之一，不足以授权开启
生产模型组开关；后续真实费用复核又触发了下述暂停决定。

## 真实费用复核与暂停决定

2026-08-31 使用生产模型组配置发出的三次真实搜索请求（Nekro `exec_code`
70976–70978），OpenRouter 最终账单分别为 `$0.006383`、`$0.0087685` 和
`$0.0070585`，平均 `$0.007403333`。同一时间窗口附近 27 次无搜索请求平均
`$0.001973960`，搜索请求约为其 3.75 倍。

其中 `$0.0070585` 请求的账单可由供应商 usage 精确闭合：11,289 input tokens 按
`$0.50/M` 计为 `$0.0056445`，138 output tokens 按 `$3/M` 计为 `$0.000414`，一次
Parallel Turbo 搜索为 `$0.001`。三项相加没有未解释余额。主要成本来自 server tool
请求重新处理完整 system/persona/history 且 cache hit 为 0；减少搜索结果字符数不能消除
这部分输入成本。

该成本结构未通过路线图的产品门。R3.1 自此暂停，所有生产模型组开关保持关闭；技术实现
和管理界面开关可以保留，但它们不表示生产可用、产品签署或待直接放量。只有新的明确决定
重新立项后，才继续以下退出条件。
