...
0.8.0 把 OneBot 已推送的群名片/群名称、成员进退、管理员、禁言、头衔、精华、群文件、
好友与入群请求、好友新增和机器人离线事件归一化为有界的事实动作。群名片和群名称变化
同时进入 Core 的名称历史；普通消息补充当时可见的群头衔和等级。缺字段会明确标为
`partial`/`unavailable`，不会猜测，也不会保存完整原始 payload。此版本不轮询成员、
不处理请求、不改变 QQ 状态，也不修改 ChatExporter。
