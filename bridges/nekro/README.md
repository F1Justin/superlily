...
1.3.0 与 Lily bridge 使用同一份 OneBot 事实归一化规则，保存群名片/群名称、成员进退、
管理员、禁言、头衔、精华、群文件、好友与入群请求、好友新增和机器人离线事件。群名片
和群名称变化进入 Core 的名称历史；普通消息从当前 OneBot 事件补充发送者当时的群头衔、
等级和角色。本版本只观察，不轮询成员、不处理请求、不产生新的 QQ 副作用，也不修改
ChatExporter。
